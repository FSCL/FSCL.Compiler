...
[<ReflectedDefinition>]
let sumElements(a: float32[], b:float32[], wi:WorkItemInfo) =    
    let gid = wi.GlobalID(0)
    a.[gid] + b.[gid]
...
let ``Can compile kernel with utility functions`` () =
    ...
    for f in result.Functions do
        if (f.Value.ParsedSignature.Name = "sumElements") then
            Assert.AreEqual(2, f.Value.OriginalParameters.Count)
        else
            Assert.AreEqual(3, f.Value.OriginalParameters.Count)
...
let ``Can compile kernel with reference cells`` () =
    ...
    Assert.AreEqual(size, wInfo)
    // Work item info parameter should be lifted
    Assert.AreEqual(4, result.Kernel.OriginalParameters.Count)
