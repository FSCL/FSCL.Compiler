...
type VectorTypeProvider(config: TypeProviderConfig) as this = 
    ...
    let namespaceName = "FSCL.Compiler"
            ...
            for i in [1 .. namedComponentCount] do
                ...
                for combination in componentsCombinations do
                    if (combination.Length = 1) then    
                        ...
                        if(componentCount <= 4) then
                            let propertyName = Generator.concatWithIndex componentNames combination 
                            let namedProperty = 
    ...
    let zip = createVectorTypes(thisAssembly, namespaceName, [ 2; 4 ])
